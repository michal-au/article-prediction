...
BNC_XGB_MODEL_PATH = os.path.join(SETTINGS.get('paths', 'model'), 'bnc-last', 'xgboost_400000.pkl')
...
def format_predictions_by_machlearn_model(y, trees):
    ...
    for t in trees:
        ...
        new_sents.append(print_sentence_ugly(t))
    #assert idx == len(y)
    return new_sents
...
if __name__ == '__main__':
    ...
    if args.model_type == 'xgboost':
        ...
        with codecs.open(args.output, 'w+', 'utf-8') as f:
            for sent in new_sents:
                f.write(sent + '\n')
    if args.model_type == 'bnc-xgboost':
        sents = remove_articles(get_sentences(args.input, sent_tokenize=True))
        model = pickle.load(open(BNC_XGB_MODEL_PATH, 'rb'))
        trees = parse_sents(sents)
        assert len(trees) == len(sents)
        test_x = extract_features(trees, bnc=True)
        test_y = model.predict(test_x)
        new_sents = format_predictions_by_machlearn_model(test_y, trees)
        with codecs.open(args.output, 'w+', 'utf-8') as f:
           for sent in new_sents:
    ...
    if args.model_type == 'kenlm':
        sents = remove_articles(get_sentences(args.input, sent_tokenize=True))
        sents = [s for s in sents if 'attempts to prevent this' in s.lower()]
        print sents

        print("LOADING KENLM....")
        model = kenlm.Model(KENLM_MODEL_PATH)
        print("...DONE")
        predict_sentence(model, nltk.word_tokenize(sents[0]), threshold=0, margin=0)
        exit()

        with codecs.open(args.output, 'w+', 'utf-8') as f:
            new_sents = []
            for sent in sents:
                f.write(' '.join(predict_sentence(model, nltk.word_tokenize(sent), threshold=0, margin=0)) + '\n')
